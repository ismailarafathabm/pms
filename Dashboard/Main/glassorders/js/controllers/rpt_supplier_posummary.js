...
export default function posummarysuppliers($scope,$compile) {
    ...
    $scope.access = {};
    const columnDefs = po.colssuppliersummary($scope, $compile, $scope.access);
    ...
    async function loadrpt() {
        ...
        const res = await po.posummarybysupplier();
        if (res?.msg !== 1) {
            alert(res.data);
            return;
        }
