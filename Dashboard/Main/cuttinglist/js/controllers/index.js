...
export default function Cuttinglists($scope, $compile) {    
    ...
    console.log(models.gridoptionsx());
    ...
    $scope.uploadctpdfsubmit = async () => {
        ...
        const res = await cts.POST(`cuttinglists/ctpdf.php?ctno=${currentct}&ct_no=${current_ctno}&ctprojectno=${current_pjno}`, fd);
        if (res?.msg !== 1) {
            alert(res.data);
            return;
        }
    ...
    let current_pjno = "";
    let current_ctno = "";
    $scope.uploadcuttinglist = (id,pj,ctno) => {
        $scope.ctuploads = {
            diashow: true,
            isloading : false,
        }
