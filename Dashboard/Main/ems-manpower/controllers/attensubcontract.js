...
export default function manpowerAttenSubcontract($scope,$compile) {
    ...
    async function LoadReport() {
        ...
        const res = await apis.GET(`pms-manpower/monthlysubcontract.php?stdate=${_startdate.value}&enddate=${_enddate.value}`);
        if (!res.isSuccess) {
            alert(res.msg);
            $scope.isLoading = false;
            $scope.$apply();
