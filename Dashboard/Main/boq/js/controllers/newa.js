...
export default function boqopenewa($scope, $http) {
    ...
    async function _saveBoq() {
        ...
        const res = await boq.POST('boqop/new_boqn.php', fd);
        if (res?.msg !== 1) {
            alert(res.data);
            $scope.isloading = false;
            $scope.$apply();
    ...
    //get all project BOQ's
    $scope.boqlist = [];
    async function GetAllOldBoqs($project) {
        $scope.boqlist = [];
        if ($scope.isloading) false;
        $scope.isloading = true;
        const res = await boq.GET('boqop/boqselect.php?projectcode=' + $project);
        if (res?.msg !== 1) {
            alert(res.data);
            $scope.isloading = false;
            $scope.$apply();
    ...
    $scope.oldgetboqinfo = async() => {
        // const id = $scope.boqeng.oldboqid ?? '0';
        // //console.log(id);
        // if (id === '' || id === '0') {
        //     return;
        // }
        // const res = await boq.GET(`boqop/getboqbyid.php?boqid=${id}`);
        // if (res?.msg !== 1) {
        //     return;
        // }
        ...
        // $scope.boqeng = {
        //     ...$scope.boqeng,
        //     poq_item_type : res.data.poq_item_type,
        // }
        // console.log($scope.boqeng);
        // $scope.$apply();
        // return;
