app.controller('ppwhtopp', ppwhtopp);

function ppwhtopp($scope, $http) {
    document.title = "POWDER COTTINGS REPORT [Warehouse To Paint Plant] - PMS";
    ...
    if (userinfo.user_name === 'demo' || userinfo.user_name === 'barakth') {
        columnDefs.push({
            ...
            cellRenderer: function (p) {
                return `<button class="ism-btns btn-delete" style="padding: 2px 2px;" onclick="editinfo('${p.data.pprid}')">
    ...
    columnDefs.push({
        ...
        cellRenderer: function (p) {
            return `
            <button class="ism-btns btn-normal" style="padding: 2px 2px;" onclick="recivedList('${p.data.pprid}')">
            ${p.data.tot_recive}
            </button>
            `;
        },
    ...
    function saveData() {
        if (!_sprocess) {
            ...
            alert("Plase Wait Previous Process Still Running.....")
        }
    }
    ...
    $scope.printResult = () => {
        ...
        localStorage.setItem("printppreportstitle", "WAREHOUSE TO FACTORY REPORT");
    ...
    $scope.receiptNew_submit = () => {
        if (!_process) {
            ...
            alert("Please Wait Previous Process Still Running...")
        }
    }
    ...
    $scope.getSummaryRpt = () => {
        ...
        let dia = document.getElementById('dia_summary_whtopp');
        ...
        let post_data = {
            url: `${api_url}ppwork/whtoppsummary.php`,
            method: 'GET',
            headers: {
                'content-type': undefined
            }
