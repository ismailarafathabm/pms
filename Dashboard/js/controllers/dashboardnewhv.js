app.controller('dashboardcontrollernewvh', ['$scope', '$http','$compile', function ($scope, $http,$compile) {
    console.log("its working");
    $scope.ctitle = "HANDED OVER PROJECTS";
    document.title = "HANDED OVER PROJECTS - PMS"
    ...
    var url = api_url + "Project/indexy.php";
    ...
    if (pdfAccess === true) {
        ...
        columndef.push({
            ...
            cellRenderer: function (p) {
                return p.data.hvf === '1' ? `
                <a href="${print_location}assets/project/${p.data.project_id}.pdf" download="${p.data.project_no}#${p.data.project_name} - Handing Over file " class="link">
                    <img src="${print_location}assets/pdfdownload.png?v=${v}" style="width:15px;"/>
                </a>
                ` : '-';
            },
...
app.controller('dashboardcontrollernewvhv', ['$scope', '$http','$compile', function ($scope, $http,$compile) {
    //console.log("its working");
    $scope.ctitle = "HANDED OVER VILLAS  LIST";
    document.title = "HANDED OVER VILLAS - PMS"
    const inputs = document.querySelectorAll("input");
    inputs.forEach(i => {
        i.autocomplete = "off";
    })
    ...
    var url = api_url + "Project/indexyv.php";
    all_techApprovalsType();
