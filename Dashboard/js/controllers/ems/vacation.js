...
app.controller("emp_vacations", emp_vacations);
function emp_vacations($scope) {
    ...
    const cols = emp.columnsVacation(filterParams);
    ...
    getVacatonInfos();
    async function getVacatonInfos() {
        if (!$scope.isLoadingInfo) {
            $scope.isLoadingInfo = true;
            const res = await emp.getVacaton();
            if (res?.msg !== 1) {
                alert(res.data);
                $scope.isLoadingInfo = false;
                $scope.$apply();
