<?php  
  ...
  //  echo  $goprojectid ;
   ...
   // print_r($playload->godate_d->);
    $godate = !isset($playload->godate_d->noraml) || trim($playload->godate_d->noraml) === '' ? '' : trim($playload->godate_d->noraml);
    if($godate === ''){
        header("HTTP/1.0 402 bad request");
    ...
    if((int)$gopflag >= 2){
        $goprelease = !isset($playload->goprelease_d->normal) || trim($playload->goprelease_d->normal) === '' ? '' : trim($playload->goprelease_d->normal);        
        if($goprelease === ''){
            header("HTTP/1.0 402 bad request");
        ...
        $goprelease = date_format(date_create($playload->goprelease_d->normal),'Y-m-d');
    }
    $gopreturn = date('Y-m-d');
    if((int)$gopflag === 3){
        $gopreturn = !isset($playload->gopreturn_d->normal) || trim($playload->gopreturn_d->normal) === '' ? '' : trim($playload->gopreturn_d->normal);
        if($gopreturn === ''){
            header("HTTP/1.0 402 bad request");
    ...
    if((int)$gocostingflag >= 2){
        $gocostingrelease = !isset($playload->gocostingrelease_d->normal) || trim($playload->gocostingrelease_d->normal) === '' ? '' : trim($playload->gocostingrelease_d->normal);
        if($gocostingrelease === ''){
            header("HTTP/1.0 402 bad request");
