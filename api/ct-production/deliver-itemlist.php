...
$id = !isset($_GET['outcno']) || trim($_GET['outcno']) === "" ? "" : trim($_GET['outcno']);
if($id === ""){
    header("http/1.0 409 input not found");
}
...
pms_cuttinglist_productions as ctp on ct.ct_id = ctp.ctid where rel.outcno = :outcno";
$cm = $cn->prepare($sql);
$cm->bindParam(":outcno",$id);
$cm->execute();
$rpts = [];
