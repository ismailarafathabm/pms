...
$zdata = "";
$s1 = [];
$token = $mtbl->token(20) ."". $pjcnoenc ."". date("Hisa") ;
foreach ($arr as $a) {
    if (!is_numeric($a->qty)) {
        $zmsg = "0";
    ...
    $s1[]= array(
        ...
        ":unit" => $a->units,        
        ":driver" => $driver,        
        ":estimatedate" => $status === "Delivered" ?  date_format(date_create($estimatedate), 'Y-m-d')  :  date_format(date_create($loadingdate), 'Y-m-d'),
        ":loadingdate" => date_format(date_create($loadingdate), 'Y-m-d'),
        ":estimatetositedate" => $status === "Delivered" ? date_format(date_create($estimatetositedate), 'Y-m-d') :   date_format(date_create($ascurrentdate), 'Y-m-d'),
        ...
        ":mattype" => $a->mattype,
        ":matunit" => $a->units,
        ":cuttinglistno" => $a->cuttinglistno,
    );
}

if ($zmsg === "0") {
    echo response($zmsg, $zdata);
    exit;
}
$p3 = array(
    ":st" => $date1,
