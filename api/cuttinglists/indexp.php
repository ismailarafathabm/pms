<?php 
    ...
    $projectno = !isset($_GET['projectno']) || trim($_GET['projectno']) === '' ? 0 : $_GET['projectno'];
    include_once 'ctools.php';
    $sql = "SELECT * FROM pms_cuttinglist where ctprojectno= :ctprojectno";    
    //echo $sql;
    $cm = $cn->prepare($sql);
    $cm->bindParam(":ctprojectno",$projectno);
    $cm->execute();
    $rpts = [];
    ...
    if ((string)$rpt['production_flag'] === "3") {
        ...
        $issuedate_d = $rpt['production_release_l'];
    }
