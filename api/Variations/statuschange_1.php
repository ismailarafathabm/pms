...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($auth === true) {
        if (!isset($token_variation) || $token_variation === "") {
            ...
            echo response("0", "Revision number Missing");
        } else if (!isset($status) || $status === "") {
            echo response("0", "Select Any Status");
        } else if (!isset($project_no) || $project_no === "") {
            echo response("0", "Project Informations Missing");
        } else {
            if ($status === '5') {
                if (!isset($who) && $who === '') {
                    echo response("0", "Enter Who");
                } else if (!isset($datepaid) && $datepaid === '') {
                    echo response("0", "Enter Date");
                } else {
                    if (!date_create($datepaid)) {
                        echo response("0", "Date Is not valid Format");
                    } else {
                        $d = date_create($datepaid);
                        $df = date_format($d, 'Y-m-d');
                        include_once('../../controller/Variations.php');
                        $v = new Variations($db);
                        $enc_date = $v->enc('enc', $df);
                        $enc_who = $v->enc('enc', $who);
                        $api = json_decode($v->update_revision_status($token_variation, $token_revison, $project_no, $status, $enc_who, $enc_date));
                        if ($api->msg === "1") {
                ...
                if (!is_uploaded_file($_FILES['pdffile']['tmp_name'])) {
                    ...
                    if ($file_ext !== "pdf") {
                        ...
                        $v = new Variations($db);
                        $e = $v->enc('enc', '-');
                        $api = json_decode($v->update_revision_status($token_variation, $token_revison, $project_no, $status, $e, $e));
