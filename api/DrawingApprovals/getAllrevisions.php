...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($auth === true) {
        if (!isset($data->project_no) || $data->project_no === '') {
            ...
            !isset($data->drawing_no) || $data->drawing_no === ''
        ) {
            echo response('0', "Choose Any Drawing Number");
        }else {            
            include_once("../../controller/DrawingApprovals.php");
            $DrawingApprovals = new DrawingApprovals($db);            
            echo $DrawingApprovals->get_drawing_revisions($data->project_no, $data->drawing_no);
        }
    } else {
