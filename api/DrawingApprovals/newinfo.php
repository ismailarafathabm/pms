...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($auth === true) {
        ...
        if ($_s === '1') {
            ...
            $date = date('Y-m-d');
            $_token = $DrawingApprovals->token(90);
            $sql = "SELECT *From pms_drawing_approvals_info";
            $cm = $db->prepare($sql);
            $cm->execute();
            $rowcnt = $cm->rowCount();
            for ($i = 0; $i < $rowcnt; $i++) {
                $sql = "SELECT *FROM pms_drawing_approvals_info where approvals_info_token='$_token'";
                $cm = $db->prepare($sql);
                $cm->execute();
                if ($cm->rowCount() === 0) {
                    break;
            ...
            $_token = $_token.$DrawingApprovals->enc('enc',$project_code).date('Ymdhis');
            $sdata = array(
                'approvals_info_token' => $_token,
                'approvals_info_project_id' => $project_code,
                'approvals_info_drawing_no' => $approvals_draw_no,
                ...
                'approvals_info_cby' => $user_name,
                'approvals_info_eby' => $user_name,
                'approvals_info_cdate' => date('Y-m-d'),
                'approvals_info_edate' => date('Y-m-d'),
                'approvals_info_remarks' => $approvals_info_remarks
            );
            $api = json_decode($DrawingApprovals->new_approvals_info($sdata));
